...
public class ClientEventHandler {
    ...
    /*
     * @SideOnly(Side.CLIENT)
     * @SubscribeEvent public void onGuiOpen(GuiOpenEvent event) { if(Minecraft.getMinecraft().thePlayer!=null &&
     * event.gui instanceof GuiResearchBrowser) {
     * if(ThaumcraftApiHelper.isResearchComplete(Minecraft.getMinecraft().thePlayer.getCommandSenderName(),
     * "WGFAKEELDRITCHMINOR")) ResearchCategories.researchCategories.get("WITCHGADG").background =
     * WGResearch.wgbackgrounds[1]; else ResearchCategories.researchCategories.get("WITCHGADG").background =
    ...
    public void initializeIcons(TextureStitchEvent.Post event) {
        if (Minecraft.getMinecraft().thePlayer != null) {
            if (ThaumcraftApiHelper
                    .isResearchComplete(Minecraft.getMinecraft().thePlayer.getCommandSenderName(), "WGELDRITCHBASE"))
