...
public class ItemInfusedGem extends Item implements IInfusedGem {
    ...
    List<ChunkCoordinates> getOres(World world, int x, int y, int z) {
        ...
        while (closedList.size() < closedListMax && !openList.isEmpty()) {
            ...
            for (ChunkCoordinates cc2 : getConnected(next.posX, next.posY, next.posZ)) if (!checked.contains(cc2)
                    && !closedList.contains(cc2)
                    && !openList.contains(cc2)
                    && (world.getBlock(cc2.posX, cc2.posY, cc2.posZ).equals(search)
                            || Utilities.isOre(world, cc2.posX, cc2.posY, cc2.posZ))/* .getMaterial()==Material.rock */)
